...
class KLSEPromptManager:
    ...
    def format_starting_prompt(self) -> str:
        ...
        prompt = f"""You are a professional-grade portfolio strategist specializing in the Malaysian equity market. I have exactly {context['total_equity_myr']:,.0f} MYR and I want you to build the strongest possible stock portfolio using only board lot positions (100-share multiples) in KLSE-listed micro-cap stocks (market cap under 300 million MYR).
...
Research and create your optimal Malaysian micro-cap portfolio."""
    ...
    def format_weekly_review_prompt(self, week_number: int, previous_thesis: str = "") -> str:
        """Format weekly portfolio review prompt with Malaysian market context"""
        context = self.generate_current_context()
        
        prompt = f"""WEEK {week_number} MALAYSIAN MARKET PORTFOLIO REVIEW

Reevaluate your Malaysian micro-cap portfolio based on latest KLSE market developments.

CURRENT PORTFOLIO STATUS:
- Total Equity: {context['total_equity_myr']:,.2f} MYR
- Cash Available: {context['cash_balance_myr']:,.2f} MYR  
- Current Return: {context['current_return_pct']:+.2f}%
- Active Positions: {context['positions_count']}/10
...
Research current Malaysian market conditions and decide on any portfolio adjustments. Focus on Malaysian micro-caps only, considering board lot requirements and local market dynamics."""
    ...
    def format_deep_research_prompt(self, available_cash: float, last_thesis: str = "") -> str:
...
PORTFOLIO STATUS:
- Available Capital: {available_cash:,.2f} MYR
